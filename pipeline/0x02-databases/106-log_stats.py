...
if __name__ == "__main__":
    ...
    for method in methods:
        num_method = logs_collection.count_documents(
            {"method": method})
        print("\tmethod {}: {}".format(
            method, num_method))
    ...
    filter_path = {
        "method": "GET",
        "path": "/status"}
    num_path = logs_collection.count_documents(
        filter_path)
    ...
    for ip in ips:
        list_ips.append(ip)
    sorted_ips = sorted(
        list_ips, key=lambda i: i["count"], reverse=True)
    ...
    if len(sorted_ips) < 10:
        ...
        limi______________________________________________________________________________________t = len(
            sorted_ips)
